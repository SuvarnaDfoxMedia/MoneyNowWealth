...
------------------------- */
cmsPageSchema.plugin(capitalizePlugin, {
  except: [
    // SEO/slug fields
    "slug",
    "page_code",
  ...
  descriptionFields: ["sections.content", "faqs.answer"],
});
...
// -------------------------
cmsPageSchema.pre<Query<ICmsPage[], ICmsPage>>(
  /^find/,
  function (this: Query<ICmsPage[], ICmsPage>, next: (err?: any) => void) {
...
cmsPageSchema.pre(
  ...
  async function (this: ICmsPage, next: (err?: any) => void) {
...
cmsPageSchema.statics.softDelete = async function (
  id: string,
): Promise<ICmsPage> {
