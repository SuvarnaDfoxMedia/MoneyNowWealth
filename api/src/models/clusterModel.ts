...
//     const existing = await mongoose.models.Cluster.findOne({
//       slug,
//       _id: { $ne: this._id },
//     });
...
// ----------------------------- */
// clusterSchema.post<ICluster>("save", async function (doc) {
...
//     //  FIX: Use updateOne instead of save() to avoid infinite loop
//     await mongoose.models.Cluster.updateOne(
//       { _id: doc._id },
//       { $set: { cluster_code: `CL${number}` } },
//     );

//     // Don't call doc.save() here - that causes infinite loop
//   }
// });
...
// clusterSchema.statics.softDelete = async function (
//   id: string,
// ): Promise<ICluster> {
...
----------------------------- */
clusterSchema.plugin(capitalizePlugin, {
  except: [
    // SEO/slug fields
    "slug",
    "cluster_code",
