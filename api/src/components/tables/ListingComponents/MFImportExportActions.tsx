...
export default function MFImportExportActions({
  ...
  const selectedLabel = useMemo(
    () => options.find((option) => option.value === selectedEntity)?.label || "Selected",
  ...
  const submitImport = async (validateOnly: boolean) => {
    ...
    try {
      const response = await axiosInstance.post<{ data: ImportReport; message: string }>(
      ...
      setInlineNotice({
        tone: "success",
        title: validateOnly ? "Validation completed" : "Import completed",
        message: validateOnly
          ? "Review the summary below before confirming the import."
          ...
          <div className="w-full max-w-3xl rounded-2xl bg-white p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Upload the Excel file, review the validation summary, and then confirm the import when everything looks right.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 rounded-xl border border-gray-200 bg-gray-50 p-4">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-600">
                    Importing into <span className="font-medium text-gray-900">{selectedLabel}</span>
                  </p>
                </div>
              ...
              <div className="mt-4 rounded-2xl border border-gray-200 bg-white p-4">
                ...
                <p className="mt-3 text-xs text-gray-500">
                  Upload a valid `.xlsx` or `.xls` file for the selected import type.
                </p>
              </div>
              ...
              <div className="mt-4 rounded-xl bg-white p-4 ring-1 ring-gray-200">
                ...
                <div className="mt-2 space-y-1 text-sm text-gray-600">
                  <p>Funds require `scheme_code` and NFOs require `nfo_id` for strict matching.</p>
                  <p>Use the exported sample for the selected type so the correct sheet names and columns are preserved.</p>
                  <p>Reference sheets are included to help with valid main category, category, and AMC names.</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 flex flex-wrap justify-end gap-3">
              ...
              <button
                ...
                disabled={!file || isImporting || isValidating}
            ...
            {report && summaryTotals && (
              <div className="mt-6 rounded-xl border border-gray-200 bg-white p-4">
                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-700">
                  ...
                  <span><strong>Sheets:</strong> {report.processedSheets.join(", ") || "-"}</span>
                </div>

                <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
                  <div className="rounded-lg bg-green-50 p-3 text-sm text-green-800">
                    <div className="font-medium">Inserted</div>
                    <div className="mt-1 text-lg font-semibold">{summaryTotals.inserted}</div>
                  </div>
                  <div className="rounded-lg bg-blue-50 p-3 text-sm text-blue-800">
                    <div className="font-medium">Updated</div>
                    <div className="mt-1 text-lg font-semibold">{summaryTotals.updated}</div>
                  </div>
                  <div className="rounded-lg bg-gray-100 p-3 text-sm text-gray-800">
                    <div className="font-medium">Skipped</div>
                    <div className="mt-1 text-lg font-semibold">{summaryTotals.skipped}</div>
                  </div>
                  <div className="rounded-lg bg-red-50 p-3 text-sm text-red-800">
                    <div className="font-medium">Errors</div>
                    <div className="mt-1 text-lg font-semibold">{summaryTotals.errors}</div>
                  </div>
                </div>
