...
const quickPrompts = ["Personal loan docs?", "EMI for 5 Lakh", "Approval time?"];
...
export default function ChatbotLayout() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !data.reply) throw new Error(data.error || "Service unavailable.");
  ...
  return (
    <main className="fixed bottom-6 right-6 z-[9999] font-sans flex flex-col items-end">
      {/* Chat Window */}
      <div 
        ref={chatRef}
        className={`flex flex-col bg-white shadow-2xl transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) overflow-hidden border border-slate-200
      ...
      >
        ...
        <header className="flex items-center justify-between px-5 py-4 bg-[#033261] text-white shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold tracking-tight">Money Now Wealth</h2>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-medium uppercase tracking-wider opacity-90">Assistant Online</span>
              </div>
            </div>
          </div>
          
          <div className="flex items-center gap-1">
            <button onClick={() => setMessages(starterMessages)} className="p-2 hover:bg-white/10 rounded-full transition-all" title="Reset Chat">
              <RotateCcw size={16} />
            </button>
            <button onClick={() => setIsFullScreen(!isFullScreen)} className="hidden md:block p-2 hover:bg-white/10 rounded-full transition-all" title="Toggle Fullscreen">
              <Maximize2 size={16} />
            </button>
            <button onClick={() => setIsExpanded(false)} className="p-2 hover:bg-white/10 rounded-full transition-all">
              <X size={20} />
            </button>
          </div>
        </header>
      ...
      {!isExpanded && (
        <button 
          ...
          className="flex items-center gap-3 bg-[#033261] text-white pl-4 pr-6 py-3.5 rounded-xl shadow-[0_10px_25px_-5px_rgba(5,150,105,0.4)] hover:scale-105 active:scale-95 transition-all group"
        >
          ...
          <span className="font-bold text-sm tracking-wide">Chat with Money Now</span>
        </button>
      )}
    </main>
  );
