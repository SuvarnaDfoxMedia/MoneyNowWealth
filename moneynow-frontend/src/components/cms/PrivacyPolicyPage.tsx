...
const PrivacyPolicyPage = ({ data }: { data: any }) => {
  ...
  return (
    <div className="font-poppins mb-16 bg-white">
      {/* CMSBanner with Dynamic bgImage Path */}
      <CMSBanner 
        title={data.title || "Privacy Policy"} 
        bgImage="/images/privacy-bg.png" 
      />

      {/* Main Content - Centered 10 Column Layout for LG */}
          ...
          {data.sections?.map((section: any, index: number) => (
            <div
              ...
              className={`rounded-[14px] overflow-hidden transition-all duration-300 ${
                openIndex === index ? "bg-[#043F79]/5 shadow-sm" : "bg-[#043F79]/5"
              }`}
            >
              ...
              <div
                className={`overflow-hidden transition-all duration-500 ease-in-out ${
                  openIndex === index ? "max-h-[4000px] opacity-100" : "max-h-0 opacity-0"
                }`}
              >
                <div className="px-6 pb-8 md:px-8 md:pb-10">
                  ...
                  <div className="bg-white rounded-xl p-8 md:p-12 border border-gray-100 shadow-sm">
                    ...
                    {/* Header specific to the first section */}
                    {index === 0 && (
                      <div className="mb-8 text-[15px] md:text-[16px] space-y-1 text-black border-b border-gray-100 pb-6 leading-[28px]">
                        <p className="font-bold uppercase tracking-wide">MONEYNOW – {data.title || "Privacy Policy"}</p>
                        <p className="pt-4 font-medium capitalize">{data.title || "Privacy Policy"}</p>
