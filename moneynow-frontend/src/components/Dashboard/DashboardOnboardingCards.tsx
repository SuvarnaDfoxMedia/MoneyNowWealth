    ...
    <section className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-3">
      {onboardingCards.map((card) => (
        <DashboardCard key={card.id} hover className="rounded-3xl p-5">
          <div className={`mb-4 grid h-12 w-12 place-items-center rounded-2xl ${card.iconBg}`}>
            <DashboardIcon name={card.icon} className="h-5 w-5 text-white" />
          </div>
          <h3 className="text-[18px] font-semibold leading-[1.35] text-[#050D1F]">{card.title}</h3>
          <p className="mt-2 min-h-[58px] text-[15px] leading-8 text-[#6374A2]">{card.description}</p>
