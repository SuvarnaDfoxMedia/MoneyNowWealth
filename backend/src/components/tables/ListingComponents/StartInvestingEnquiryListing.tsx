...
interface StartInvestingEnquiry {
  _id: string;
  full_name: string;
  email: string;
  mobile: string;
...
export default function StartInvestingEnquiryListing() {
  const location = useLocation();
  const MODULE_KEY = "admin-start-investing-enquiries";
  ...
  useEffect(() => {
    ...
    if (
      storedPath &&
      !storedPath.includes("/start-investing-enquiry") &&
      currentPath.includes("/start-investing-enquiry")
    ) {
      markTabSwitch();
    }
  ...
  useEffect(() => {
    if (!isMounted) return;
    void markModulesAsRead(["start-investing-enquiries"]);
  ...
  useEffect(() => {
    const handleNavClick = () => {
      setTimeout(() => {
        ...
        if (!currentPath.includes("/start-investing-enquiry")) {
          markTabSwitch();
        }
      }, 100);
    };
  ...
  const { data, extractList, isLoading, refetch, deleteRecord } =
    useCommonCrud<StartInvestingEnquiry>({
      module: "start-investing-enquiries",
      role: "admin",
      page,
  ...
  const [enquiries, setEnquiries] = useState<StartInvestingEnquiry[]>([]);
  ...
  const columns: TableColumn<StartInvestingEnquiry>[] = [
    {
      key: "index",
      label: "#",
    ...
    { key: "calculator_type", label: "Calculator Used", sortable: true, render: (row) => row.calculator_type || "None" },
    {
      key: "created_at",
      label: "Date",
      sortable: true,
