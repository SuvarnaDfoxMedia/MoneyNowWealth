...
interface WhoWeWorkWithEnquiry {
  _id: string;
  full_name: string;
  email: string;
  mobile: string;
...
export default function WhoWeWorkWithEnquiryListing() {
  const location = useLocation();
  const MODULE_KEY = "admin-who-we-work-with-enquiries";
  ...
  useEffect(() => {
    ...
    if (
      storedPath &&
      !storedPath.includes("/who-we-work-with-enquiry") &&
      currentPath.includes("/who-we-work-with-enquiry")
    ) {
      markTabSwitch();
    }
  ...
  useEffect(() => {
    const handleNavClick = () => {
      setTimeout(() => {
        ...
        if (!currentPath.includes("/who-we-work-with-enquiry")) {
          markTabSwitch();
        }
      }, 100);
    };
  ...
  const { data, extractList, isLoading, refetch, deleteRecord } =
    useCommonCrud<WhoWeWorkWithEnquiry>({
      module: "who-we-work-with-enquiries",
      role: "admin",
      page,
  ...
  const [enquiries, setEnquiries] = useState<WhoWeWorkWithEnquiry[]>([]);
  ...
  const columns: TableColumn<WhoWeWorkWithEnquiry>[] = [
    {
      key: "index",
      label: "#",
    ...
    { key: "persona_label", label: "Persona", sortable: true },
    { key: "preference", label: "Preference", sortable: true },
    {
      key: "created_at",
      label: "Date",
      sortable: true,
          ...
          <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/70 px-4">
            <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
              ...
              <p className="mt-2 text-sm text-gray-600">
                Are you sure you want to delete this persona enquiry?
              </p>
