...
import { FiMoreVertical, FiTrash2, FiDownload } from "react-icons/fi";
...
interface PortfolioReviewEnquiry {
  _id: string;
  full_name: string;
  email: string;
  mobile: string;
  country_code?: string;
  investor_mindset: string;
...
export default function PortfolioReviewEnquiryListing() {
  const location = useLocation();
  const MODULE_KEY = "admin-portfolio-review-enquiries";
  ...
  useEffect(() => {
    ...
    if (
      storedPath &&
      !storedPath.includes("/portfolio-review-enquiry") &&
      currentPath.includes("/portfolio-review-enquiry")
    ) {
      markTabSwitch();
    }
  ...
  useEffect(() => {
    if (!isMounted) return;
    void markModulesAsRead(["portfolio-review-enquiries"]);
  ...
  useEffect(() => {
    const handleNavClick = () => {
      setTimeout(() => {
        ...
        if (!currentPath.includes("/portfolio-review-enquiry")) {
          markTabSwitch();
        }
      }, 100);
    };
  ...
  const { data, extractList, isLoading, refetch, deleteRecord } =
    useCommonCrud<PortfolioReviewEnquiry>({
      module: "portfolio-review-enquiries",
      role: "admin",
      page,
  ...
  const [enquiries, setEnquiries] = useState<PortfolioReviewEnquiry[]>([]);
  ...
  const columns: TableColumn<PortfolioReviewEnquiry>[] = [
    {
      key: "index",
      label: "#",
    ...
    { key: "email", label: "Email", sortable: true },
    {
      key: "mobile",
      label: "Mobile",
      render: (row) => `${row.country_code ? `${row.country_code} ` : ""}${row.mobile}`.trim() || "N/A",
    },
    { key: "investor_mindset", label: "Mindset", sortable: true },
    { 
      key: "cas_file", 
      label: "CAS Statement", 
      render: (row) => row.cas_file_url ? (
        <a 
          href={row.cas_file_url} 
          target="_blank" 
          rel="noreferrer"
