...
export default function AddMFTopHolding() {
  ...
  const addHoldingRow = () =>
    setHoldings((prev) => [emptyHoldingRow(), ...prev]);
  ...
  const validate = () => {
    ...
    if (!form.portfolio_date)
      nextErrors.portfolio_date = "Portfolio date is required";
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    const payload = {
      ...
      portfolio_date: form.portfolio_date
        ? form.portfolio_date.toISOString()
        : null,
      ...
      assets_top_10_holdings_pct: toNumberOrNull(
