...
export default function MFTopHoldingDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const response = await axiosApi.getOne<TopHoldingDetail>(`/${role}/mf/top-holdings/${id}`);
  ...
  const columns: TableColumn<HoldingEntry>[] = [
    ...
    { key: "sector", label: "Sector", render: (row) => formatValue(row.sector) },
    { key: "maturity", label: "Maturity", render: (row) => formatValue(row.maturity) },
    {
      key: "credit_quality_india",
      label: "Credit Quality",
      render: (row) => formatValue(row.credit_quality_india),
    },
    { key: "country", label: "Country", render: (row) => formatValue(row.country) },
    {
      key: "actions",
      label: "Actions",
      render: (row) => (
        ...
        <div className="space-y-5">
          <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
            <div className="grid gap-4 md:grid-cols-3">
              <div>
                <p className="text-xs font-medium uppercase text-gray-500">Fund</p>
                <p className="mt-1 font-medium text-gray-900">{record.fund_name}</p>
              </div>
              <div>
                <p className="text-xs font-medium uppercase text-gray-500">Scheme Code</p>
                <p className="mt-1 text-gray-800">{formatValue(record.scheme_code)}</p>
              </div>
              <div>
                <p className="text-xs font-medium uppercase text-gray-500">Source ISIN</p>
                <p className="mt-1 text-gray-800">{formatValue(record.source_isin)}</p>
              </div>
              <div>
                <p className="text-xs font-medium uppercase text-gray-500">Portfolio Date</p>
                <p className="mt-1 text-gray-800">{formatDate(record.portfolio_date)}</p>
              </div>
              <div>
                <p className="text-xs font-medium uppercase text-gray-500">Uploaded</p>
                <p className="mt-1 text-gray-800">{formatDate(record.uploaded_at)}</p>
              </div>
              <div>
                <p className="text-xs font-medium uppercase text-gray-500">Status</p>
                <p className="mt-1 text-gray-800">{record.is_active === 1 ? "Active" : "Inactive"}</p>
              </div>
            </div>
          </div>
