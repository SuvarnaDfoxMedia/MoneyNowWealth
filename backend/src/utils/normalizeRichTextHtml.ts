...
export const hasLooseBulletLines = (html: string) => {
  if (typeof document === "undefined" || !html?.trim()) return false;

  const root = document.createElement("div");
  root.innerHTML = html;

  return Array.from(root.querySelectorAll(BLOCK_SELECTOR)).some((element) => {
    if (element.closest("ul, ol")) return false;
    const lines = extractLineHtmlSegments(element as HTMLElement);
    return lines.some((line) => {
      const temp = document.createElement("div");
      temp.innerHTML = line;
      return hasBulletMarker(temp.textContent ?? "");
    });
  });
};
